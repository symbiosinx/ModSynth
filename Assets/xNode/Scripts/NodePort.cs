...
namespace XNode {
    ...
    public class NodePort {
        ...
        public object GetOutputAudioValue(double time) {
            if (direction == IO.Input) return null;
            return node.GetAudioValue(this, time);
        }

        public object GetInputAudioValue(double time) {
            NodePort connectedPort = Connection;
            if (connectedPort == null) return null;
            return connectedPort.GetOutputAudioValue(time);
        }
        ...
        public T GetInputAudioValue<T>(double time) {
            object obj = GetInputAudioValue(time);
            return obj is T ? (T) obj : default(T);
        }
