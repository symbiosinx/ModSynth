...
#region Inputs/Outputs
        ...
        public T GetInputAudioValue<T>(string fieldName, double time, T fallback = default(T)) {
            NodePort port = GetPort(fieldName);
            if (port != null && port.IsConnected) return port.GetInputAudioValue<T>(time);
            else return fallback;
        }

        public virtual object GetAudioValue(NodePort port, double time) {
            // Debug.LogWarning("No GetAudioValue(NodePort port, double time) override defined for " + GetType());
            return null;
        }
#endregion
